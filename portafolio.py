...
pip install pdf2image streamlit
pip install fpdf2 pdf2image Flask
...
if selected == 'Reporte':
  ...
  if analysis == 'Equipos':
    ...
    winrate = (filter_df["result"].sum() / len(filter_df)) * 100
  ...
  if len(option2) <1:
    ...
    filter_df['KDA'] = (filter_df['kills'] + filter_df['assists']) / filter_df['deaths']
    ...
    with descripcion3:
      ...
      en este caso podemos ver que tiene un mejor kda es: **{player_with_max_KDA}** con **{max_KDA}**
      ''')
  ...
  if len(option2) < 1:
    ...
    sun = px.sunburst(
        df,
        path=['side', 'split', 'league', 'teamname', 'playername', 'champion'],
        values='result',
        color='side',
...
if selected == 'Feedback':
  ...
  sheet_name = "DataBase League of Legends"
  sheet1= gc.open(sheet_name).get_worksheet(3)
  ...
  with st.form(key='feedback_form'):
      ...
      calificacion = st.slider("Calificación del 1 al 5:", 1, 5, 5)
  ...
  if submit_button:
    ...
    if not nombre:
      ...
      sheet1.append_row([nombre, email, comentario, calificacion])
